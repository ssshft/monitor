...
LarkRebot::LarkRebot() {
	url = MonitorConfig::GetInstance().GetLarkUrl();
	authorization = MonitorConfig::GetInstance().GetLarkAuthorization();
	channel = MonitorConfig::GetInstance().GetLarkChannel();
...
void LarkRebot::SendLarkMsg(MsgCard& msgCard, string larUrl) {
	try {
		web::http::client::http_client_config config;
		config.set_timeout(utility::seconds(5));
		web::http::client::http_client restclient(larUrl, config);
...
void LarkRebot::SendVoiceCall(string msg, string receiveId) {
	...
	//if (currentTime - lastSendTime <= voiceCallInterval * 60 * 1000) {
	//	LOG_INFO("SendVoiceCall time interval < %f min  receiveId: %s", voiceCallInterval, receiveId.c_str());
	//	return;
	//}
...
void LarkRebot::SendGroupVoiceCall(string msg, string groupId, vector<string>& vUserId) {
	...
	//if (currentTime - lastSendTime <= voiceCallInterval * 60 * 1000) {
	//	LOG_INFO("SendGroupVoiceCall time interval < %f min  groupId: %s", voiceCallInterval, groupId.c_str());
	//	return;
	//}
