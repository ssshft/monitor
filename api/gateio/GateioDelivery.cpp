...
GateioDelivery::GateioDelivery(AccountInfo& info) {
    accountUrl = "/api/v4/delivery/";
    positionUrl = "/api/v4/delivery/";
    orderUrl = "/api/v4/delivery/";
    accountInfo = info;
}
...
bool GateioDelivery::QueryAccount(vector<gateio::FutureAsset>& vFutureAsset, vector<string>& vErrorMsg) {
    bool query = true;
    vector<string> vSettle{"usdt", "btc"};
...
bool GateioDelivery::QueryPosition(vector<gateio::FuturePosition>& vFuturePosition, vector<string>& vErrorMsg) {
    bool query = true;
    vector<string> vSettle{"usdt", "btc"};
    for (size_t i = 0; i < vSettle.size(); ++i) {
...
bool GateioDelivery::QueryOpenOrder(vector<gateio::FutureOrder>& vFutureOrder, vector<string>& vErrorMsg) {
    bool query = true;
    vector<string> vSettle{"usdt", "btc"};
    for (size_t i = 0; i < vSettle.size(); ++i) {
...
bool GateioDelivery::QuerySettleAccount(string settle, vector<gateio::FutureAsset>& vFutureAsset, vector<string>& vErrorMsg) {
    int count = 0;
    bool query = true;
    while (count < 3) {
        query = true;
        ...
        try {
            ...
            .then([](http_response response) -> pplx::task<json::value> {  // if the status is OK extract the body of the response into a JSON value
                ...
                LOG_INFO("GateioDelivery QueryAccount response: '%s' ", response.to_string().c_str());
                throw exception();
            ...
            .then([&](pplx::task<json::value> previousTask) {  // get the JSON value from the task and display content from it
                ...
                if (futureAsset.total >= 0.0000000001) {
            	    LOG_INFO("QueryAccount AccountId: %d   GateioDelivery asset: %s", accountInfo.accountId, futureAsset.toString().c_str());
		    ...
		    if (label != "USER_NOT_FOUND") {
                        ...
                        ss << "GateioDelivery QueryAccount msg:" << msg;
                        string errMsg = ss.str();
                        LOG_INFO("QueryAccount AccountId: %d   GateioDelivery Error: '%s' ", accountInfo.accountId, errMsg.c_str());
            ...
            query = false;
            string errMsg = string("GateioDelivery QueryAccount ") + string(e.what());
            LOG_INFO("QueryAccount AccountId: %d   GateioDelivery Error: '%s' ", accountInfo.accountId, errMsg.c_str());
            vErrorMsg.emplace_back(errMsg);
        }
...
bool GateioDelivery::QuerySettlePosition(string settle, vector<gateio::FuturePosition>& vFuturePosition, vector<string>& vErrorMsg) {
    int count = 0;
    bool query = true;
    while (count < 3) {
        query = true;
        ...
        try {
            ...
            .then([](http_response response) -> pplx::task<json::value> {  // if the status is OK extract the body of the response into a JSON value
                ...
                LOG_INFO("GateioDelivery QueryPosition response: '%s' ", response.to_string().c_str());
                throw exception();
                return pplx::task_from_result(json::value());  // return an empty JSON value
            })
            .then([&](pplx::task<json::value> previousTask) {  // get the JSON value from the task and display content from it
                ...
                if (content.is_array()) {
                    ...
                    for (auto& position: positionArray) {
            		    ...
            		    LOG_INFO("QueryPosition AccountId: %d   QuerySettlePosition position: %s  settle: %s", accountInfo.accountId, futurePosition.toString().c_str(), settle.c_str());
		    ...
		    if (label != "USER_NOT_FOUND") {
                    ...
                    ss << "GateioDelivery QueryPosition msg:" << msg;
                    string errMsg = ss.str();
                    LOG_INFO("QueryPosition AccountId: %d   GateioDelivery Error: '%s' ", accountInfo.accountId, errMsg.c_str());
                    vErrorMsg.emplace_back(errMsg);
                }
            ...
            query = false;
            string errMsg = string("GateioDelivery QueryPosition") + string(e.what());
            LOG_INFO("QueryPosition AccountId: %d   QuerySettlePosition Error: '%s' ", accountInfo.accountId, errMsg.c_str());
            vErrorMsg.emplace_back(errMsg);
        }
...
bool GateioDelivery::QuerySettleOpenOrder(string settle, vector<gateio::FutureOrder>& vFutureOrder, vector<string>& vErrorMsg) {
    int count = 0;
    bool query = true;
    while (count < 3) {
        query = true;
        ...
        try {
            ...
            .then([](http_response response) -> pplx::task<json::value> {  // if the status is OK extract the body of the response into a JSON value
                ...
                LOG_INFO("GateioDelivery QueryOpenOrder response: '%s' ", response.to_string().c_str());
                throw exception();
                return pplx::task_from_result(json::value());  // return an empty JSON value
            })
            .then([&](pplx::task<json::value> previousTask) {  // get the JSON value from the task and display content from it
                ...
                if (content.is_array()) {
            		...
            		LOG_INFO("QueryOpenOrder AccountId: %d   GateioDelivery order: %s", accountInfo.accountId, futureOrder.toString().c_str());
		    ...
		    if (label != "USER_NOT_FOUND") {
                    	...
                    	ss << "GateioDelivery QueryOpenOrder msg:" << msg;
                    	string errMsg = ss.str();
                    	LOG_INFO("QueryOpenOrder AccountId: %d   GateioDelivery Error: '%s' ", accountInfo.accountId, errMsg.c_str());
                    	vErrorMsg.emplace_back(errMsg);
		    }
                }
            })
            ...
            query = false;
            string errMsg = string("GateioDelivery QueryOpenOrder ") + string(e.what());
            LOG_INFO("QueryOpenOrder AccountId: %d   GateioDelivery Error: '%s' ", accountInfo.accountId, errMsg.c_str());
            vErrorMsg.emplace_back(errMsg);
        }
