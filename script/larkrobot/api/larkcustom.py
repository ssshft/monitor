...
class LarkCustomBot(object):
    ...
    async def post(self, data):
        ...
        log_engine.info(data)
        data = json.dumps(data)
        try:
            response = requests.post(webhook, headers=headers, data=data)
        except requests.exceptions.HTTPError as exc:
            log_engine.error("消息发送失败， HTTP error: %d, reason: %s" %
                          (exc.response.status_code, exc.response.reason))
            raise
        except requests.exceptions.ConnectionError:
            log_engine.error("消息发送失败，HTTP connection error!")
            raise
        except requests.exceptions.Timeout:
            log_engine.error("消息发送失败，Timeout error!")
            raise
        except requests.exceptions.RequestException:
            log_engine.error("消息发送失败, Request Exception!")
            raise
        else:
            ...
            if result.get('code') is not None:
                log_engine.error("消息发送失败，自动通知：%s" % result)
            # 发送成功 返回数据会有StatusCode
            if result.get('StatusCode') == 0:
                log_engine.info('发送成功')
